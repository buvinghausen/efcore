...
**Date:** 2026-08-14 (rev. 2 after design review)
**Author:** Brian Buvinghausen (with Claude)
**Status:** Draft — direction approved; query mechanism and workstream B metadata shape must be proven by the spike gates in §7 before implementation planning
...
- **Store-object resolution.** New overloads in `SqlServerEntityTypeExtensions`: `IsTemporal(this IReadOnlyEntityType, in StoreObjectIdentifier)` — fragment override wins, else the entity-level value (D2). The design also declares (tier 1 defines, later tiers consume): `GetHistoryTableName/Schema(entityType, storeObject)` and period-column-name store-object overloads on the same pattern.
- **Table-level reader — consensus contract, not `.First()`.** `SqlServerTableExtensions.IsTemporal(this ITable)` must be safe when an `ITable` carries multiple entity-type mappings (table splitting, owned types sharing the table, entity-splitting fragments). Contract: resolve `IsTemporal(entityType, storeObject)` for **every** mapping on the table; validation (§3.2) guarantees all mappings agree — the existing `ValidateTemporalTableSplitting` (`SqlServerModelValidator.cs:627–690`) already enforces exactly this consensus for table-sharing types (all root types temporal, period start/end column names identical), and the new fragment validation extends the same guarantee to fragments. The reader then returns the validated consensus; history/period metadata is selected from a defined canonical mapping (the principal mapping of the first root type, ordering pinned by tests), never from arbitrary enumeration order. Shared-table and owned-type combinations are explicit test cases (§6).
...
4. **Reject survivors.** A postprocessing check (SQL Server query translation postprocessor) walks the pruned tree: if any surviving `TableExpression` maps to a non-temporal fragment of an entity that has temporal-annotated tables in the same query, throw the purpose-built error naming the member(s)/table and the remedy ("project members mapped to the principal table").
...
- Table-level reader consensus: shared-table (table splitting) + owned-types-sharing-table + fragment combinations, pinning canonical-mapping selection order.
- Migrations: `MigrationsSqlServerTest.TemporalTables.cs` — the **full §3.3 acceptance-gate matrix**.
...
3. **Workstream B PR** — provider-neutral, anchored on #27972/#27971.
...
| Temporal fragments (tier 2) | each fragment | fragment-level `GetHistoryTableName/Schema(storeObject)` (declared §3.1, unconsumed) | per-fragment period columns — property-mapping design needed | Open: period properties per fragment |
